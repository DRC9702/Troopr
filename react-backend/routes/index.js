...
const MyComponent = React.createFactory(require('../components/MyComponent'));

/* GET home page. */
router.get('/api', User.middleware.loadAll, (req, res, next) => {
  res.json({
    user: req.users,
  });
});

router.get('/apiT', Team.middleware.loadAll, (req, res, next) => {
  res.json({
    user: req.teams,
...
router.post('/api/profile', User.middleware.loadOfLog, (req, res, next) => {
...
router.post('/api/edit_profile', User.middleware.loadOfLog, (req, res, next) => {
...
router.post('/api/create_account', User.middleware.loadAll, (req, res, next) => {
...
router.post('/api/create_event', (req, res, next) => {
  if (!req.session.user) {
    res.json({
      success: false,
      login: false,
  ...
  const start_date_para = req.body.start_date.split('/');
  const start_date = new Date(parseInt(start_date_para[2]), parseInt(start_date_para[0]) - 1, parseInt(start_date_para[1]), 0, 0, 0);
  const end_date_para = req.body.end_date.split('/');
  const end_date = new Date(parseInt(end_date_para[2]), parseInt(end_date_para[0]) - 1, parseInt(end_date_para[1]), 0, 0, 0);
  const registration_deadline_para = req.body.registration_deadline.split('/');
  const registration_deadline = new Date(parseInt(registration_deadline_para[2]), parseInt(registration_deadline_para[0]) - 1, parseInt(registration_deadline_para[1]), 0, 0, 0);
  ...
  Event1.add(fields, (error, event) => {
    ...
    User.findById(req.session.user._id, (error1, user) => {
      ...
      const event_list = user.eventsHosted ? user.eventsHosted : [];
...
router.post('/api/show_event', (req, res, next) => {
  if (!req.session.user) {
    res.json({
      success: false,
      login: false,
...
router.post('/api/search_event', Event1.middleware.loadAll, (req, res, next) => {
  if (!req.body.query) {
    if (req.events) {
      res.json({
        success: 'success',
  ...
  if (req.events) {
    const event_list = req.events.filter(p => strContains(p.host, req.body.query) || strContains(p.name, req.body.query));
    res.json({
      success: 'success',
      events: event_list,
    });
...
router.post('/api/sign_in', credential.middleware.loadOfEmail, User.middleware.loadOfCre, (req, res, next) => {
  const email = req.body.email;
  const password = req.body.password;
  if (!(email && password)) {
    res.json({
...
router.post('/api/create_profile', (req, res, next) => {
...
router.get('/addProfile', (req, res, next) => {
  ...
  Profile.add(fields, (error, pro) => {
    ...
    credential.add(fields2, (error, cre) => {
      if (error) {
        res.json({
          success: 'fail at cre',
        });
...
router.post('/api/view_team', (req, res, next) => {
  if (!req.session.user) {
    res.json({
      success: false,
...
router.post('/api/user_teams', (req, res, next) => {
  if (!req.session.user) {
    res.json({
      success: false,
      message: 'Need login first.',
...
router.post('/api/edit_team', (req, res, next) => {
  if (!req.session.user) {
    res.json({
      success: false,
      message: 'Need login first.',
  ...
  Team.findByEventIdAndUseId(req.body.event_id, req.session.user._id, (error, team) => {
    if (error || !team) {
      ...
      console.log(fields);
      Team.update(team._id, fields, (error) => {
        if (error) {
          res.json({
            success: false,
...
router.post('/api/join_event', Event1.middleware.loadOfId, Team.middleware.loadAll, (req, res, next) => {
    ...
    if (error1) {
      ...
      Team.add(field, (error, Tea) => {
        if (error) {
          ...
          Event1.update(req.event._id, fields, (error) => {
            if (error) {
              res.json({
                success: false,
...
router.post('/api/give_team', Team.middleware.loadOfEvent, (req, res, next) => {
  console.log(req.body);
...
router.post('/api/reject_team', Team.middleware.loadOfEvent, (req, res, next) => {
    ...
    if (error || !team) {
      ...
      Team.update(req.body.team_id, {
        ...
        if (error) {
          ...
          Team.findById(req.body.team_id, (error, team23) => {
            console.log(team23);
            res.json({
              success: true,
            });
...
router.post('/api/team_matched', Event1.middleware.loadOfId, (req, res, next) => {
  if (!req.session.user) {
    res.json({
      success: false,
      login: false,
  ...
  Team.findById(req.body.team1, (error, team1) => {
    if (error || !team1) {
      ...
      Team.findById(req.body.team2, (error, team2) => {
        if (error || !team2) {
          res.json({
            success: false,
